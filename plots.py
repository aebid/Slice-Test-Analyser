...
###################################################DENSITY###################################################
#############################################################################################################
plot2Ddensity("(rechit_localx_GE11 - prop_localx_GE11):prop_localy_GE11:prop_localx_GE11", \
              "prop_prop_res_center", event, [50, -30, 30, 50, -15, 15], "", "", \
              "abs(rechit_prop_dX_GE11) < 10", "res_density")
plot2Ddensity("rechit_prop_dX_GE11:prop_localy_GE11:prop_localx_GE11", "prop_prop_res", event, \
              [50, -30, 30, 50, -15, 15], "", "", "abs(rechit_prop_dX_GE11) < 10", "res_density")
plot2Ddensity("rechit_prop_dphi_GE11:prop_localy_GE11:prop_localx_GE11", "prop_prop_res_phi", event, \
              [50, -30, 30, 50, -15, 15], "", "", "abs(rechit_prop_dphi_GE11) < .5", "res_density")

for i in range(2):
  for j in range(1,37):
    plot2Ddensity("rechit_prop_dX_GE11[{}]:prop_localy_GE11[{}]:prop_localx_GE11[{}]".format(i,i,i), \
                  "prop_prop_res_layer{}_chamber{}".format(i,j), event, [50, -30, 30, 50, -15, 15], \
                  "local x(cm)", "local y(cm)", \
                  "abs(rechit_prop_dX_GE11) < 10 && chamber_GE11[{}] == {}".format(i, j), "res_density_LC")
############################################################################################################
  ...
  for j in range(1,37):
    for k in range(1,9):
      plot1Dhist("rechit_prop_dX_GE11[{}]".format(i), "dx_layer{}_chamber{}_roll{}".format(i,j,k), event, \
                 [100, -40, 40], "x(cm)", "", \
                 "chamber_GE11[{}] == {} && roll_GE11[{}] == {}".format(i,j,i,k), "dx_localx_LCR")
#############################################################################################################
...
for i in range(2):
  for j in range(1,9):
    plot1Dhist("rechit_prop_dX_GE11[{}]".format(i), "Local_res_layer{}_roll{}".format(i,k), event, \
               [100, -4, 4], "x(cm)", "", "roll_GE11[{}] == {}".format(i,k), "dx_localx_LR")
    plot1Dhist("rechit_localx_GE11[{}] - prop_localx_center_GE11[{}]".format(i,i), \
               "Local_res_center_layer{}_roll{}".format(i,k), event, [100, -4, 4], "x(cm)", "", \
               "roll_GE11[{}] == {}".format(i,k), "dx_localx_LR_center")
#############################################################################################################
